...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    //반드시 username으로 설정해야 i값이 넘어옴(name 소문자)
    //pw input 태그의 name 속성 값을 password로 지정하면
    //해당 input 태그의 데이터를 비밀번호로 간주함


    //로그인을 실행하는 url이 호출되면 아래의 메소드가 자동으로 실행
    //로그인 하려는 사람의 로그인에 필요한 정보를 조회해서 시큐리티한테 전달(return)
    //하는 코드를 작성
    @Override
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        //로그인 하려는 회원의 정보 조회
        MemberVO member = boardService.login(username); //로그인 하는 사람의 정보만 조회

        //조회한 정보를 userDetails 타입으로 변환(UserDetails 자료형으로 return 시켜 주기 위해서)
