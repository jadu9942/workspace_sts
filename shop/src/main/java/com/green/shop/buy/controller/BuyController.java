...
public class BuyController {
    ...
    public String buyCarts(CartVO cartVO, HttpSession session) {
        ...
        for (CartViewVO e : cartViewList) {
            buyPrice = buyPrice + e.getTotalPrice(); // 총 구매가격
        }
        ...
        buyVO.setBuyCode(buyCode);//매개변수로 들어가는 buyCode값 넣어주기
        ...
        for (CartViewVO cartView : cartViewList) {
            BuyDetailVO vo = new BuyDetailVO(); // cartView 행 하나
        ...
        buyVO.setBuyDetailList(buyDetailList); //상세 정보
    ...
    public String history(@RequestParam(name="page",required = false, defaultValue = "history")String page, Model model, HttpSession session){
        //page로 넘어오는 값이 없으면 history로 default
        model.addAttribute("page",page);

        //로그인 정보 조회
        MemberVO loginInfo= (MemberVO) session.getAttribute("loginInfo");

        //구매 목록 조회 (구매정보 다 들어가있음)
        ...
        return "content/buy/history";  //구매 이력 확인
    }
